...
class Net(nn.Module):
    def __init__(self, level, s, in_channels, input_shape, cond_channels,
        ...
        self.change_time_channel = nn.Conv3d(2**(2*(level+1))*c, 2**(2*(level+1))*c,
                                             (3,3,3), stride=(2,1,1),padding=(0,1,1))  # nn.Conv3d(3, 1, kernel_size=1) # conv = nn.Conv3d(4, 1, (3,3,3), stride=(1,1,1),padding=(0,0,0))
        ...
        self.Net = nn.Sequential(
                   ...
                   nn.Conv3d(intermediate_size, intermediate_size,
                             kernel_size=3, padding=1),
                   nn.ReLU(),
                   nn.Conv3d(intermediate_size, in_channels, #  2**(2*(level+1))//2 *c,
                             kernel_size=1),
                   nn.ReLU())
...
class ConditionalCoupling(nn.Module):
    ...
    def forward(self, z, logdet=0, logpz=0, reverse=False):
...
class GaussianPrior(nn.Module):
    def __init__(self, C, s, level, cond_channels, flow_var_shape, final=False):
        ...
        if final:
            ...
            self.change_time_channel = nn.Conv3d(2*C, 2*C, (1,1,1), stride=(2,1,1),
                                                 padding=(0,0,0))
    ...
    def forward(self, x, all_latents=None, reverse=False, eps=1.0, logpz=0,
