...
def crps_ensemble(observation, forecasts):
    ...
    for i in range(fc.shape[1]): # in case we have more channels. Now implemented for case where c=1.
        below = fc_below # fc_below[:,i,...]
        weight = ((i+1)**2 - i**2) / fc.shape[-1]**2
        crps[below] += weight * (obs[below]- fc[below])

    for i in range(fc.shape[1]-1,-1,-1):
        above  = ~fc_below
        k = fc.shape[1]-1-i
        weight = ((k+1)**2 - k**2) / fc.shape[1]**2
        crps[above] += weight * (fc[above]-obs[above])
